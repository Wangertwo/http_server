...
struct callback 
{
    ...
    callback(F&& f) 
        : m_base(std::make_unique<_callback_impl<std::decay_t<F>>>(std::forward<F>(f)))
    {}
    
    callback(std::nullptr_t) noexcept {}
    
    void operator()(Args... args) /* callshot once */
    {
        assert(m_base);
    ...
    void operator()(multishot_call_t, Args... args) /* multishot */
    {
        assert(m_base);
        m_base->_call(std::forward<Args>(args)...);
    }
